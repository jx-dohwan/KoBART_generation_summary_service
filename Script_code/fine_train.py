...
def define_argparser():
    ...
    p.add_argument('--post_train_notapply', type=bool, default=False)
...
def main(config):
    ...
    train_tokenize_data = train_data.map(partial(preprocess_data, config=config, tokenizer=tokenizer), batched = True, remove_columns=['Text', 'Summary'])
    val_tokenize_data = val_data.map(partial(preprocess_data, config=config, tokenizer=tokenizer), batched = True, remove_columns=['Text', 'Summary'])
